...
// right-to-left defaults
    ...
    $.fn.updateFullCalendar = function (markers) {
        this.each(function () {
            markers.forEach((m, i) => {
    ...
    $.fn.fullCalendar = function (options) {
        ...
        if (options.renderViewEvent)
            renderViewEvent = options.renderViewEvent;
            ...
            if (calendarMarkers) {
                ...
                if (marker) {
                    ...
                    function getTooltip(markers) {
                        let tooltip = '<ul>';
                        markers.forEach((m) => {
                            tooltip += '<li><i class=\'fas fa-caret-' + (m[1] ? 'up' : 'down') + ' pr-1\'></i>' + m[0] + '</li>'
                        });
                        tooltip += '</ul>';
                        return tooltip;
                    }

                    if (marker.minor.length) {
                        content = '<div class="col-12"><span class="react-tag tag-minor" data-toggle="tooltip" ' +
                            'data-html="true" title="' + getTooltip(marker.minor) + '">Minori: <span class="tag-number">' +
                            marker.minor.length + '</span></span></div>';
                    }
                    if (marker.medium.length)
                        content += '<div class="col-12"><span class="react-tag tag-medium" data-toggle="tooltip" ' +
                            'data-html="true" title="' + getTooltip(marker.medium) + '">Medie : <span class="tag-number">' +
                            marker.medium.length + '</span></span></div>';
                    if (marker.major.length)
                        content += '<div class="col-12"><span class="react-tag tag-major" data-toggle="tooltip" ' +
                            'data-html="true" title="' + getTooltip(marker.major) + '">Gravi : <span class="tag-number">' +
                            marker.major.length + '</span></span></div>';
                }
            }
            html += "<div class='fc-day-content container'><div class='row'>" + content + "</div></div>" +
                "</td>";
	...
	-----------------------------------------------------------*/
        ...
        function setHeight(height) {
            ...
            bodyFirstCells.each(function (i, _cell) {
                if (i < rowCnt) {
                    cell = $(_cell);
                    let minHeight = (i == rowCnt - 1 ? rowHeightLast : rowHeight) - vsides(cell);
                    if (minHeight > 140)
                        minHeight = 140;
                    cell.find('> div').css('min-height', minHeight);
                }
            });
